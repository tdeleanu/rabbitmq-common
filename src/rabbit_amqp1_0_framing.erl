...
keys(Record) ->
    [{symbol, symbolify(K)} || K <- rabbit_framing_amqp_1_0:fields(Record)].
...
decode({described, Descriptor, {list, Fields}}) ->
    case rabbit_framing_amqp_1_0:record_for(Descriptor) of
        #'v1_0.amqp_sequence'{} ->
...
decode({described, Descriptor, Field}) ->
    fill_from(rabbit_framing_amqp_1_0:record_for(Descriptor), Field);
decode(null) ->
    undefined;
decode(Other) ->
     Other.
...
encode(X) ->
    rabbit_framing_amqp_1_0:encode(X).
...
symbol_for(X) ->
    rabbit_framing_amqp_1_0:symbol_for(X).

number_for(X) ->
    rabbit_framing_amqp_1_0:number_for(X).

pprint(Thing) when is_tuple(Thing) ->
    case rabbit_framing_amqp_1_0:fields(Thing) of
        unknown -> Thing;
